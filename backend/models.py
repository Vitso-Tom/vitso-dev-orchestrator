...
class Job(Base):
    ...
    # === NEW: Adopted Projects ===
    # Adopted projects are externally-managed codebases registered with VDO
    # VDO can deploy/stop them but cannot rebuild (code lives outside VDO)
    is_adopted = Column(Boolean, default=False)
    adopted_path = Column(String(500), nullable=True)  # External codebase location
    startup_command = Column(String(500), nullable=True)  # Custom startup command
    ...
    # === NEW: Local Deployment (Demo Feature) ===
    deploy_local = Column(Boolean, default=False)  # User requested deployment
    deployment_pid = Column(Integer, nullable=True)  # Process ID
    deployment_port = Column(Integer, nullable=True)  # Assigned port
    deployment_url = Column(String(200), nullable=True)  # Access URL
    deployment_output_dir = Column(String(500), nullable=True)  # Files location
    deployment_type = Column(String(50), nullable=True)  # python/node/static
